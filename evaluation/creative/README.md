...
`ad_production_reference` 컬렉션의 upsert/조회 + v1 파일 legacy 정규화(구 `element_vector_store.py`)와
...
이 폴더가 추출을 담당하는 컬렉션은 `ad_production_reference` 하나다(연출/프로덕션 디테일 —
컨셉 확정 후 M5~M9·스토리보드 HTML 참고용, 실제 적재/조회 로직은 `db/chromadb/importers/
production_reference.py`). 전략 레퍼런스 컬렉션 `ad_concept_reference`(M3 컨셉 발산 참고용)는
[`../concept/README.md`](../concept/README.md)와 `db/chromadb/importers/concept_reference.py`
가 별도로 관리한다 — 두 컬렉션의 용도 구분은 [`../README.md`](../README.md)의 스키마 통합
계획 참고.
...
(`db/chromadb/importers/production_reference.py::_profile_metadata`) `PRODUCTION_SEGMENT_COLUMNS`
(`persuasion_engine`/`narrative_form`)로 직접 필터링하거나 `cliche_aggregate._aggregate_casting`
분포 집계에 잡힌다.
...
`ad_concept_reference`/`ad_production_reference` 참조 광고 검색(세그먼트 필터·self-reference
정책·호출 로깅 포함)과 그걸 MCP 도구로 노출하는 `chromadb-explorer` 서버는
[`../../db/README.md`](../../db/README.md)에서 통합 관리한다(`db/chromadb/creative_search.py`,
`db/chromadb/mcp_server.py`) — `generation/v5_m0_m3` 의 `--retrieval` 옵션, self-reference
정책(`--self_video_id`/`--self_mode`)도 그 문서에 사용법이 있다
([`../../generation/v5_m0_m3/README.md`](../../generation/v5_m0_m3/README.md) 참고).
