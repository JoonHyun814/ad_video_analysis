...
- [ ] 🤖 성분·효능 콜아웃 씬 존재 (`text_overlay`에 성분명·수치 포함)
- [ ] 🤖 엔드컷에 패키지 + 브랜드명 + 슬로건 3요소가 함께 언급됨
...
- [ ] 🤖 `brand` 필드에 브랜드명과 제품명이 정확히 기재됨 (오탈자 없음)
- [ ] 🤖 핵심 성분 키워드(예: "해마추출물")가 `key_messages` 또는 씬 description에 등장
- [ ] 🤖 핵심 기능 키워드(예: "리프팅", "화이트닝")가 `key_messages`에 명시됨
- [ ] 🧠 `key_messages`가 브리프의 제품 USP와 의미적으로 일치하는가
- [ ] 🧠 씬의 분위기·음악·톤이 브리프의 타겟 연령대 감성에 부합하는가
...
- [ ] 🧑 `cast` 캐릭터 연령·이미지 설정이 브리프의 타겟 페르소나와 부합
...
- [ ] 🧑 프리미엄 포지셔닝인 경우, 저가 이미지를 주는 요소(과도한 세일·가격 강조 등)가 없는가
...
- [ ] 🧠 슬로건/태그라인이 브랜드 포지셔닝과 논리적으로 부합하는가
