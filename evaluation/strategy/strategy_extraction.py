...
from evaluation.strategy.strategy_schemas import (
    M1_GUIDE,
    M1_SCHEMA,
    M2_GUIDE,
    M2_SCHEMA,
...
_COMMON_HEADER = (
    ...
    "[공통 규칙]\n"
    "- 역추론은 창작이 아니다: 시나리오에서 실제 관찰되는 장면·대사·자막·연출과 부합하는 내용만 채워라.\n"
    "  이 광고에 없는 새 아이디어·메시지·컨셉을 지어내지 마라.\n"
    "- 서술 어투: 산출물은 광고 제작 '전에' 작성된 전략 문서처럼 쓴다. '이 광고는/영상을 보니/\n"
    "  시나리오에서/N번째 컷에서' 같은 분석·관찰 어투와 컷 번호 언급을 절대 쓰지 마라.\n"
    "  내용은 실제 광고와 부합하되, 문장은 전략을 제안하는 형식이어야 한다.\n"
    "- 근거 표기 필드(source·evidence 등)에는 대사·카피 원문만 간결히 적는다\n"
    "  (예: \"대사 '아직 한 통도 안 썼는데' 기반\") — '영상/광고를 보면' 류 표현 금지.\n"
    "- ★'광고', '영상', '시나리오', '컷'이라는 단어를 rationale 등 모든 필드에서 쓰지 마라.\n"
    "  (예: '광고가 거실 장면에서 출발해 …'(금지) → '거실 채널 경합이 가장 보편적인 페인이므로 …'(허용)\n"
    "  — 근거는 소비자·시장·카피 논리로 서술한다.)\n"
    "- 관찰로 확인 불가한 추정(시장 규모·경쟁사·수치 등)에는 [가설] 태그와 신뢰도(상/중/하)를 달고,\n"
    "  근거가 전혀 없는 항목은 빈 문자열/빈 배열로 남겨라.\n"
