...
| 파일 | 역할 |
|------|------|
| `connection.py` | 클라이언트/컬렉션 연결 헬퍼 + 임베딩 함수(`BAAI/bge-m3`, `EMBEDDING_MODEL`/`get_embedding_function`) — 이 저장소의 모든 컬렉션이 공유하는 단일 소스 |
| `list_collections.py` | 컬렉션(테이블) 목록 + 레코드 수 출력 |
| `show_schema.py` | 컬렉션 하나 지정 → 메타데이터 스키마(필드·타입·예시) + 데이터 수 출력 |
| `show_by_video_id.py` | 컬렉션 + `video_id` 지정 → 해당 레코드 전체 출력 |
| `search_query.py` | 컬렉션 + 자연어 쿼리 지정 → 유사도 상위 레코드 출력(범용) |
| `tool_definitions.py` | 위 4개 조회 유틸을 Anthropic tool_use 스키마(`TOOL_DEFINITIONS`)+디스패처(`call_tool`)로 감싼 공유 모듈(저장소 자동 탐색 포함) |
| `creative_search.py` | `ad_concept_reference`/`ad_production_reference` 의미 검색(세그먼트 필터·self-reference 정책·검색 로그 포함) — RAG 백엔드. `generation/retrieval_pipeline`·`generation/v5_m0_m3 --retrieval`가 이걸 쓴다 |
| `mcp_server.py` | 위 조회 유틸 + creative_search 를 MCP 도구 8개로 노출하는 stdio MCP 서버(`chromadb-explorer`, 저장소 루트 `.mcp.json` 등록) |
| `importers/category.py` | `<data_root>/<video_id>/category_analysis.json` → `data/category` 컬렉션 적재(전체 필드) — 독립 CLI |
| `importers/scenario.py` | `<data_root>/<video_id>/scenario_analysis.json` → `data/scenario` 컬렉션 적재(concept/narrative/key_messages/production_notes + cast·scenes 개수) — 독립 CLI |
...
자연어 검색용 ChromaDB 컬렉션에 적재한다. 다른 ChromaDB 컬렉션과 달리 **전용 저장 경로**를
...
```bash
python -m db.chromadb.importers.category [--data_root output/total] [--db_path data/category] [--rebuild]
python -m db.chromadb.importers.scenario [--data_root output/total] [--db_path data/scenario] [--rebuild]
```
...
이 모듈은 **코드에서 직접 호출하는 라이브러리**이자 **MCP/Anthropic tool_use 도구 정의의
단일 소스**다(`TOOL_DEFINITIONS_CONCEPT`/`TOOL_DEFINITIONS_PRODUCTION`/`call_tool`):

- `generation/retrieval_pipeline/retrieval.py` — M5(결정적 검색 실행)가 직접 호출
- `generation/v5_m0_m3/llm_adapter.py --retrieval` — MCP(`chromadb-explorer`, cli 백엔드) 또는
  Anthropic 네이티브 tool_use(api 백엔드)로 LLM에 노출

## MCP 서버 / Claude API 도구 — `chromadb-explorer`

`db.chromadb.*` 조회 유틸 4개(list_collections/show_schema/show_by_video_id/search_query) +
`creative_search.py` 참조 광고 검색 4개, 총 8개 도구를 Claude CLI(`claude -p`/대화형 세션)와
Claude API 양쪽에 노출한다. 이 저장소의 유일한 ChromaDB MCP 서버다. `importers/category.py`·
`importers/scenario.py`·평가 파이프라인 적재 모듈(컬렉션 삭제·재적재 배치 작업)은 도구로
...
| `search_chromadb` | `collection`(필수), `query_text`(필수, 자연어), `n_results`(기본 5) | 유사도 상위 레코드(범용) |
| `list_concept_segment_columns` | 없음 | `ad_concept_reference` 필터 컬럼/허용값 |
| `search_concept_reference` | `query_text`(필수), `segment_column`/`segment_value`(선택, exact-match), `top_k`(기본 5, 최대 20) | 전략적으로 비슷한 참조 광고(소구·포지셔닝·타겟) |
| `list_production_segment_columns` | 없음 | `ad_production_reference` 필터 컬럼/허용값 |
| `search_production_reference` | `query_text`(필수), `segment_column`/`segment_value`(선택, exact-match), `top_k`(기본 5, 최대 20) | 연출이 비슷한 참조 광고 + 대표 크리에이티브 요소 |

앞 4개는 `db_path` 를 도구 인자로 받지 않는다 — `collection` 명만 주면
`tool_definitions._resolve_db_path` 가 알려진 3개 저장소를 훑어 자동으로 찾는다(호출하는
쪽이 내부 폴더 구조를 몰라도 됨). 뒤 4개(`*_concept_reference`/`*_production_reference`)는
`output/vector_db` 고정이다.
...
HTTP/SSE MCP 커넥터만 지원), 범용 4개는 `db.chromadb.tool_definitions.TOOL_DEFINITIONS`/
`call_tool`, 참조 광고 검색 4개는 `db.chromadb.creative_search.TOOL_DEFINITIONS_CONCEPT`/
`TOOL_DEFINITIONS_PRODUCTION`/`call_tool` 을 그대로 가져다 `messages.create(..., tools=...)`
호출과 tool_use 왕복 루프에 쓴다 — `generation/v5_m0_m3/llm_adapter.py::_chat_json_api_with_tools`
가 하는 일이 이 패턴이다.

**임베딩 모델 예열**: `mcp_server.py` 는 `__main__` 실행 시 `creative_search.warm_up()` 으로
bge-m3 와 `ad_concept_reference`/`ad_production_reference` 컬렉션을 서버 기동 시점에 미리
로드한다 — 그렇지 않으면 첫 검색 호출이 모델 로딩 비용까지 떠안아 느려지거나 타임아웃에
걸릴 수 있다.
