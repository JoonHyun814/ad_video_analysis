...
def analyze_cuts_openai(
    cuts: list[Cut],
    frames_dir: Path,
    ocr_data: dict[str, list[str]],
    model: str = DEFAULT_MODEL,
) -> list[dict]:
    """각 컷의 프레임 시퀀스를 OpenAI Vision으로 분석하고, 모델이 추가 분할한 sub_cuts를 펼쳐 반환한다."""
...
def _analyze_one(
    ...
    raw = call_openai_with_images(prompt, image_paths, model=model, timeout=180)
