...
def _run_video(args: argparse.Namespace, video_id: int | None, video_path_override: Path | None = None) -> None:
    ...
    if args.skip_preprocess:
        ...
        if video_path_override is not None:
            print(f"[1/10] 영상 파일 확인 중... ({video_path_override})")
            video_path = video_path_override
            print(f"      파일: {video_path}")
        else:
            print(f"[1/10] 영상 정보 조회 중... (video_id={video_id})")
    ...
    if args.skip_scene_analysis:
        ...
        print(f"[8/10] Scene 분석 중... (gemini, 컷 수={len(cuts)})")
    ...
    if args.skip_cut_analysis:
        ...
        print(f"[9/10] Cut 분석 중... (gemini, 컷 수={len(cuts)})")
