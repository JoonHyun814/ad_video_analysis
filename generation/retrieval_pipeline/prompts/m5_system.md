...
아래 M4 시나리오(등장인물/컷별 구성 — 각 컷은 `visual`(화면구성)/`motion`(동적 연출)/
`dialogue`(대사)/`narration`(나레이션)/`subtitle`(자막)/`sound`(사운드) 6개 필드로 이루어져
있다)와 M0 제품 사실을 참고해, 정확히 아래 구조를 완성하라. `visual`·`motion`만 시각 정보다
— `dialogue`·`narration`·`subtitle`·`sound`는 이미지/영상 생성 대상이 아니므로 무시하라
(대사·나레이션·자막·사운드는 별도 더빙/자막/음향 단계의 몫이다).
...
- `keyframe_image_prompt`: 그 컷의 대표 순간 하나를 정지 이미지로 그리는 프롬프트. 해당 컷의
  `visual`(공간·구도·인물/제품 배치)을 반영한 구도 묘사. 등장하는 인물/제품/공간은 위
  1)·2)·3)에서 고정한 묘사와 반드시 일치시켜라
- `seedance_prompt`: 그 키프레임에서 영상이 어떻게 전개되는지만 서술 — 해당 컷의 `motion`
  (카메라 무브먼트·인물 동작)이 시간에 따라 어떻게 진행되는지(동작의 시작→끝, 돌리인·패닝·
  틸트 등 카메라 움직임 포함)를 명시하라. 인물 외형·제품 형태·공간 디테일 같은 정적 정보는
  절대 반복하지 마라 — 오직 "무엇이 움직이는가"만 써라
