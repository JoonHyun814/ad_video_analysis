"""retrieval_pipeline 오케스트레이터.
  ...
  run_m3(): M2가 완성한 장치 8개 중 2~4개씩 조합해 러프한 시나리오 초안 5개를 만든다(LLM 1회,
      ...
      요청 — "m2의 device를 2~4개정도 조합해서 러프한 시나리오를 5개정도 생성"). M3→M4 배선
      (초안 중 하나를 골라 M4에 넘기는 것)은 아직 하지 않았다 — 다음 요청에서 다룬다.
  run_m4(): M2가 완성한 장치 8개 중 이 제품·광고 길이에 맞는 것을 골라 조합해, 광고 전체
      시나리오(scenario_analysis.json 과 동일한 구조)를 완성한다(LLM 1회, search_chromadb
      호출은 선택적, scenario_generation.py). 지금은 M3(러프 초안)를 거치지 않고 M2 산출물을
      직접 받는다 — M3와는 독립적인 별개 경로다(위 run_m3() 설명 참고).
...
def run_m4(module0: dict, m1: dict, m2: dict, context: dict, creative_problem: str,
    ...
    """M4 — M2가 완성한 장치 8개 중 이 제품·광고 길이에 맞는 것을 골라 조합해 광고 전체
    시나리오(cast/scenes/key_messages/production_notes — scenario_analysis.json 과 동일한
    구조)를 완성한다. M3(러프 초안 5개, run_m3() 참고)와는 독립적인 별개 경로다 — 지금은
    M2 산출물을 직접 받는다.

    module0/context/creative_problem/devices 는 모두 m2.json 을 그대로 펼친 값이다(cli_m4.py
    ...
    m2.json 에는 module0 원본이 더 이상 저장되지 않으므로(run_m2() 참고) cli_m4.py 는 보통
    빈 dict 를 넘긴다 — 이 우회는 module0 이 있을 때만 의미가 있다.

    log_prefix/log_dir: run_m2()와 동일한 관례 — cli_m4.py 는 m2.json 이 있던 실행 폴더를
