"""M5 — 쿼리별 검색을 실제로 실행한다(결정적, LLM 도구 호출이 아니라 순수 함수 호출).

query_scout.py(M4) 가 제안한 query_text/target_collection 을 그대로 evaluation/ad_concept_production
...
from generation.retrieval_pipeline.schemas import SearchQuery

_DEFAULT_TOP_K = 3
_DEFAULT_ELEMENTS_PER_AD = 3


def _search_one(query: SearchQuery, *, top_k: int, elements_per_ad: int, db_path: str) -> dict[str, Any]:
    collection = query.target_collection if query.target_collection == "concept" else "production"
    if collection == "concept":
        result = search_concept_reference(query.query_text, top_k=top_k, db_path=db_path)
    else:
        result = search_production_reference(
            query.query_text, top_k=top_k, elements_per_ad=elements_per_ad, db_path=db_path,
        )
    return {
        "axis": query.axis,
        "label": query.label,
...
def run_searches(queries: list[SearchQuery], *, top_k: int = _DEFAULT_TOP_K,
                 elements_per_ad: int = _DEFAULT_ELEMENTS_PER_AD,
                 db_path: str = "output/vector_db") -> list[dict[str, Any]]:
    """쿼리마다 검색 1건씩 실행해 (쿼리+결과)를 묶어 반환한다 — 순서는 queries 순서 그대로."""
    return [_search_one(q, top_k=top_k, elements_per_ad=elements_per_ad, db_path=db_path) for q in queries]


def queries_only(searches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """"서칭에 입력되는 쿼리"만 뽑은 뷰 — 실제 검색 결과 없이 무엇을 물었는지만 기록."""
    return [
        {"label": s["label"], "axis": s["axis"], "collection": s["collection"],
...
def results_only(searches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """"서칭 결과로 나온 데이터"만 뽑은 뷰 — 어느 쿼리로 나온 결과인지 태그만 남기고 원문 그대로."""
    return [
        {"label": s["label"], "axis": s["axis"], "collection": s["collection"],
