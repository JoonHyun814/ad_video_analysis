"""retrieval_pipeline CLI — M3(M1 인사이트 분석 + search_chromadb 자율 호출로 연출 장치 8개 완성).

M1(cli_m1.py) 다음 단계이자, 이 파이프라인이 두 번째로 자체 LLM 호출을 수행하는 단계다.
--m1_input(cli_m1.py 가 만든 m1.json — 제품 종류/외관/사용법/기능/재료/브랜드 이미지/타겟/
기타사항)이 이제 M3의 주 근거다(사용자 요청 — "M3는 이제 m0_m2 말고 m1을 토대로 작동"). 한 줄
...
legacy m1(JTBD 인사이트)/m2(포지셔닝 성명서)로 M3 맥락을 더 보강하고 싶을 때만 넘긴다. 하나도
없이는 실행할 수 없으므로 --input 과 --m1_input 중 최소 하나는 있어야 한다.

출력 폴더: --m1_input 을 주면 그 m1.json 이 있던 폴더(cli_m1.py 가 만든 실행 폴더)에 그대로
m3.json 을 이어서 저장한다(cli_m4.py/cli_m5.py 와 같은 관례 — --title 불필요). --m1_input
없이 --input(legacy)만 쓰면 이 실행이 새 날짜 폴더(output/retrieval_pipeline/<날짜>_<제목>/)
를 만드는 단계가 되므로 --title 이 필요하다.

사용법:
    # M1 인사이트를 근거로 실행(권장) — 같은 폴더에 m3.json 이어서 저장
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="retrieval_pipeline M3 (M1 인사이트 분석 + 도구 호출로 연출 장치 8개 완성)")
    p.add_argument("--m1_input", type=Path, default=None,
                   help="cli_m1.py 가 만든 m1.json 경로 — M3의 주 근거(제품 종류/외관/사용법/기능/재료/"
...
def main() -> None:
    ...
    result = run_m3(legacy.get("module0"), legacy.get("m1"), legacy.get("m2"), m1_insight=m1_insight,
