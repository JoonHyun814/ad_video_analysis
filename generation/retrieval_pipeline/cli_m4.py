"""retrieval_pipeline CLI — M4(M2 장치 조합 → 광고 전체 시나리오 완성, scenario_analysis.json
과 동일한 구조).

M2(cli_m2.py) 다음 단계. m2.json(context + creative_problem + devices 8개, module0/m1/m2
원본은 m2.json에 더 이상 저장되지 않는다 — pipeline.py 모듈 docstring 참고)을 읽어, LLM이
그중 이 제품·광고 길이에 맞는 장치를 골라 하나의 시나리오로 조합한다 —
`search_chromadb` 도구로 비슷한 길이의 광고 컷 구성·페이싱을 참고할 수도 있다(선택, MCP로
자율 호출). M3(cli_m3.py, M2 장치 2~4개 조합 → 러프 시나리오 초안 5개)와는 독립적인 별개
경로다 — 지금은 M3를 거치지 않고 m2.json을 직접 받는다(M3 산출물 중 하나를 골라 M4로 넘기는
배선은 아직 없다, 다음 요청에서 다룬다). 새 날짜 폴더를 만들지 않고 --input 파일과 같은
디렉터리에 m4.json으로 저장한다(pipeline.py 모듈 docstring의 관례 — 이후 단계는 이어서 같은
폴더에 쌓인다).

사용법:
    python -m generation.retrieval_pipeline.cli_m4 \\
        --input output/retrieval_pipeline/<날짜>_<제목>/m2.json [--llm_backend cli|api]
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="retrieval_pipeline M4(M2 장치 조합 → 광고 전체 시나리오)")
    p.add_argument("--input", type=Path, required=True,
                   help="m2.json 경로(context/creative_problem/devices 포함, module0/m1/m2 원본은 없음)")
...
def main() -> None:
    ...
    # m2.json 은 더 이상 module0/m1/m2 원본을 담지 않는다(사용자 요청) — 있으면(구버전
    # m2.json 과의 하위호환) 쓰고, 없으면 빈 dict.
