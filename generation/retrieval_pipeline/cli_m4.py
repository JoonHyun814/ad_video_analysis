...
M0~M2(cli.py) → M3(cli_m3.py) 다음 단계. 여기서부터 M4~M7 이 독립 CLI로 나뉜다(v5_m0_m3 의
cli.py/cli_m3.py/cli_m4_m9.py 분리와 같은 이유 — 비용이 큰 LLM 호출 단계를 고정해두고 뒷 단계만
몇 번이든 다시 돌릴 수 있게, 사용자 요청). 이 실행의 출력 폴더(output/retrieval_pipeline/
<날짜>_<제목>/)를 새로 만드는 단계이기도 하다 — 이후 cli_m5/cli_m6/cli_m7 은 --input 파일과
같은 디렉터리에 이어서 저장한다(cli_m3.py 와 동일한 관례).
...
한 줄 크리에이티브 원칙의 우선순위는 다음과 같다:
    ...
    3) 아무것도 지정하지 않으면 — M3 의 concepts[] 중 rank=1(M3가 가장 적절하다고 자가평가한
       후보)을 자동으로 쓴다.
...
사용법:
    python -m generation.retrieval_pipeline.cli_m4 \\
        ...
        --title "DBH_15초_CTV" [--concept "..." | --select_concept "PAS 모델"] \\
...
def _resolve_concept_line(args: argparse.Namespace, m3: dict[str, Any]) -> str:
    ...
    if concepts:
        top = min(concepts, key=lambda c: c.get("rank", 999))
        print(f"  --concept 미지정, M3 {top.get('rank')}순위[{top.get('technique')}] 컨셉 사용: {top.get('concept_line')}")
...
def main() -> None:
    ...
    result = run_m4(data["module0"], data["m1"], data["m2"], data.get("m3", {}),
                    concept_line, ad_length=args.ad_length)
