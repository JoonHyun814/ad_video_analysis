...
M3(cli_m3.py) 다음 단계. m3.json(module0~m2 + context + creative_problem + devices 8개)을
읽어, LLM이 그중 이 제품·광고 길이에 맞는 장치를 골라 하나의 시나리오로 조합한다 —
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--input", type=Path, required=True,
                   help="m3.json 경로(module0/m1/m2/context/creative_problem/devices 포함)")
...
def main() -> None:
    ...
    result = run_m4(data["module0"], data["m1"], data["m2"], data["context"],
                    data["creative_problem"], data["devices"],
