"""M3 — m0~m2 맥락을 분석하고 search_chromadb 도구를 자율 호출해 근거를 모은 뒤 연출 장치
8개를 완성한다(LLM 1회 호출 — 그 호출 안에서 도구 왕복은 여러 번). 이 파이프라인의 이전
설계(device_scout(LLM, 검색 없음) → retrieval(코드, 결정적 검색) → synthesis(LLM, 결과 반영)
3단계)를 한 단계로 합쳤다 — "검색할지·언제·몇 건"을 LLM 이 tool_use 로 스스로 판단하게 한다
(사용자 요청 — pipeline 개편). system/user 프롬프트 문구는 전부 prompts/m3_*.md 에 있다(이
파일은 프롬프트 조립·LLM 호출·파싱만 한다).
"""
...
def run_device_generation(context: dict[str, Any], *, ad_length: str = "15초",
    ...
    if isinstance(raw, dict) and raw.get("error"):
        ...
        raise RuntimeError(f"M3(device_generation) LLM 호출 실패: {raw.get('error')} — {str(raw.get('raw', ''))[:300]}")
