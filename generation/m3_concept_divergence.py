...
def build_prompt(brief: dict, m1: dict, m2: dict) -> str:
    """브리프+M1+M2에서 M3 컨셉 발산 프롬프트를 만든다."""
...
def run(
    ...
    *,
    ...
    """컨셉 발산(M3)을 수행한다."""
    return call_llm(build_prompt(brief, m1, m2), backend=backend, gemini_model=gemini_model, codex_model=codex_model)
