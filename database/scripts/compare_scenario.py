...
_VIDEO_SOURCE_NOTES = {
    "caption": (
        " 이 영상은 나레이션 음성과 로고·QR·오버레이 문구 등 후반 합성까지 끝난 최종 납품본이다"
        " — 합성 단계에서 사람이 추가한 브랜드 로고·QR·오버레이 문구는 정상적인 후반 작업 결과이니 "
        "스토리보드에 없다는 이유만으로 결함으로 보지 말고, 스토리보드가 의도한 내용(대사·자막·장면)이 "
        "훼손되지 않았는지를 기준으로 판정해라."
    ),
    "narration": (
        " 이 영상은 나레이션 음성 합성까지 끝난 버전이다 — 대사/나레이션 판정 시 이 오디오 트랙을 "
        "기준으로 삼는다."
    ),
}
